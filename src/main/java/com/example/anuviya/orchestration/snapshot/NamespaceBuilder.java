...
import com.example.anuviya.model.namespace.NamespaceWarning;
import com.example.anuviya.context.CircularDependency;
...
public class NamespaceBuilder {
    ...
    public Map<String, NamespaceInfo> build(List<EntityInfo> entities) {
        ...
        for (NamespaceInfo ns : map.values()) {
            ...
            // Compute Abstractness Score (0.0 to 1.0)
            long totalClasses = ns.getEntities().size();
            long abstractOrInterfaceCount = ns.getEntities().stream()
                .filter(e -> e.isAbstract() || e.getKind() == com.example.anuviya.model.entity.EntityKind.INTERFACE)
                .count();
            double abstractness = 0.0;
            if (totalClasses > 0) {
                abstractness = (double) abstractOrInterfaceCount / totalClasses;
            }
            ns.setAbstractness(abstractness);

            // Compute Distance from Main Sequence (|A + I - 1|)
            double distance = Math.abs(abstractness + instability - 1.0);
            ns.setDistance(distance);
