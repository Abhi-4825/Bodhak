package com.example.anuviya.compiler.resolution;

import com.example.anuviya.compiler.CompilationUnit;
import com.example.anuviya.ir.declaration.ImportDeclaration;
import java.util.*;
...
public final class NameResolver {
    ...
    public Optional<String> resolveStaticMember(String memberName) {
        boolean isPython = cu.getFilePath().toString().endsWith(".py");
        for (ImportDeclaration imp : cu.getImports()) {
            if (imp.isStatic() || isPython) {
                if (imp.isWildcard()) {
                    // Static wildcard import: e.g. import static com.example.Capability.*;
                    // For Python: from module import *
                    ...
                    // For Python: from module import member
