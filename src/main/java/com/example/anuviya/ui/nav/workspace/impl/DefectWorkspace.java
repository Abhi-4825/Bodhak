...
public class DefectWorkspace implements Workspace {
    ...
    private final java.util.Map<AnalysisType, InvestigationPane> panes = new java.util.EnumMap<>(AnalysisType.class);
    ...
    public DefectWorkspace() {
        ...
        // Listen for platform state updates to refresh model selector
        com.example.anuviya.platform.state.PlatformState.currentProperty().addListener((obs, oldVal, newVal) -> {
            Platform.runLater(this::refreshModelSelector);
        });
    ...
    private Node buildSidebar() {
        ...
        String[] icons = {"🏛", "⚡", "📈", "🔒", "🔧"};
        boolean[] isPlaceholder = {false, true, false, true, true};
    ...
    private void refreshModelSelector() {
        ...
        com.example.anuviya.platform.state.PlatformState state = com.example.anuviya.platform.state.PlatformState.getCurrent();
        if (state != null && !state.getInstalledModels().isEmpty()) {
            for (var model : state.getInstalledModels()) {
        ...
        if (currentSelection != null && modelSelector.getItems().contains(currentSelection)) {
