...
import com.example.anuviya.ui.nav.workspace.impl.DefectView.AnalysisResult;
import com.example.anuviya.ui.nav.workspace.impl.DefectView.Defect;
...
public class InvestigationPane extends ScrollPane {
    ...
    private final VBox placeholderBox;
    ...
    public InvestigationPane(AnalysisType type) {
        this.type = type;
        setFitToWidth(true);
        ...
        // Build beautiful placeholder Box
        placeholderBox = new VBox(20);
        placeholderBox.setAlignment(Pos.CENTER);
        placeholderBox.setPadding(new Insets(40));
        placeholderBox.setStyle("-fx-background-color: #111819; -fx-border-color: #1e2526; -fx-border-radius: 12; -fx-background-radius: 12; -fx-border-width: 1;");
        placeholderBox.setMaxWidth(600);

        Label iconLbl = new Label(type == AnalysisType.ARCHITECTURE ? "⬡" : "⚡");
        iconLbl.setStyle("-fx-font-size: 48px; -fx-text-fill: #00daf3;");

        Label titleLbl = new Label(type == AnalysisType.ARCHITECTURE ? "ARCHITECTURE INTEGRITY INVESTIGATION" : "GROWTH & SCALABILITY ANALYSIS");
        titleLbl.setStyle("-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: #dde4e5; -fx-letter-spacing: 0.05em;");

        Label descLbl = new Label(type == AnalysisType.ARCHITECTURE ? 
            "Identify structural design defects, package isolation issues, class-level circular references, and Martin's stability violations." :
            "Pinpoint growth bottlenecks, high-coupling hubs, propagation risks, and deep dependency paths within your project graph.");
        descLbl.setWrapText(true);
        descLbl.setTextAlignment(TextAlignment.CENTER);
        descLbl.setStyle("-fx-text-fill: #849494; -fx-font-size: 13px; -fx-line-spacing: 4px;");

        VBox bulletBox = new VBox(8);
        bulletBox.setAlignment(Pos.CENTER_LEFT);
        bulletBox.setMaxWidth(380);
        String[] checks = type == AnalysisType.ARCHITECTURE ? new String[]{
            "✓  Martin's Package Stability (I, A, D Metrics)",
            "✓  Class & Package Circular Dependencies",
            "✓  God Classes & Oversized Source Entities",
            "✓  High-Coupling & Inter-module Cohesion"
        } : new String[]{
            "✓  Betweenness Centrality & Central Dependency Hubs",
            "✓  High Fan-In and Fan-Out Accumulations",
            "✓  Deep Dependency Depth & Propagation Limits",
            "✓  Circular Dependency Growth Bottlenecks"
        };
        for (String check : checks) {
            Label lbl = new Label(check);
            lbl.setStyle("-fx-text-fill: #8bfd91; -fx-font-size: 12px; -fx-font-family: monospace;");
            bulletBox.getChildren().add(lbl);
        }

        Label promptLbl = new Label("Click 'Run Analysis' in the left panel to begin the reasoning engine.");
        promptLbl.setStyle("-fx-text-fill: #566465; -fx-font-size: 11px; -fx-font-style: italic; -fx-padding: 10 0 0 0;");

        placeholderBox.getChildren().addAll(iconLbl, titleLbl, descLbl, bulletBox, promptLbl);
        ...
        sessionContainer.getChildren().addAll(placeholderBox, timelineFeed, statusFooter, resultsDashboardView);
    ...
    private void processEvent(AnalysisEvent event) {
        ...
        if (event.type() == AnalysisEventType.INFO) {
            ...
            currentModelBubble.startTyping(runningModel + " reasoning...\n" +
                "(Note: Local model analysis is starting. This process may take a minute or two depending on your local machine's hardware capabilities, CPU, and GPU configuration...)\n\n");
    ...
    private void populateResultsView(String findings, AnalysisType type) {
        ...
        if (findings == null || findings.isBlank() || findings.startsWith("Analysis failed")) {
            String errorMsg = (findings != null && !findings.isBlank()) ? findings : "Analysis failed: Local AI service (Ollama) returned an empty response. Please verify that Ollama is running locally and the selected model is downloaded.";
            Label errorLabel = new Label(errorMsg);
            errorLabel.setStyle("-fx-text-fill: #ff4b4b; -fx-font-size: 14px; -fx-font-weight: bold; -fx-padding: 16;");
        ...
        try {
            ...
            if (jsonContent == null || jsonContent.isBlank()) {
                Label errorLabel = new Label("Analysis failed: AI response did not contain valid JSON output. Ensure Ollama local service is active.");
                errorLabel.setStyle("-fx-text-fill: #ff4b4b; -fx-font-size: 14px; -fx-font-weight: bold; -fx-padding: 16;");
                errorLabel.setWrapText(true);
                resultsDashboardView.getChildren().add(errorLabel);
                return;
            ...
            if (result != null && result.overallAssessment != null && !result.overallAssessment.isBlank()) {
                VBox assessmentBox = new VBox(8);
                assessmentBox.setPadding(new Insets(16));
                assessmentBox.setStyle("-fx-background-color: rgba(75,246,255,0.05); -fx-background-radius: 8; -fx-border-color: rgba(75,246,255,0.2); -fx-border-radius: 8; -fx-border-width: 1;");
                Label titleLabel = new Label("OVERALL ASSESSMENT");
                titleLabel.setStyle("-fx-font-size: 10px; -fx-text-fill: #4bf6ff; -fx-font-weight: bold; -fx-letter-spacing: 0.5;");
                Label contentLabel = new Label(result.overallAssessment);
                contentLabel.setWrapText(true);
                contentLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #dde4e5; -fx-line-spacing: 4px;");
                assessmentBox.getChildren().addAll(titleLabel, contentLabel);
                resultsDashboardView.getChildren().add(assessmentBox);
            }
            ...
            if (result != null && result.defects != null && !result.defects.isEmpty()) {
                ...
                HBox summaryLayout = new HBox(20);
                summaryLayout.getChildren().addAll(
                    createSummaryCard("Total Defects", "0", "#dde4e5"),
                    createSummaryCard("High Severity", "0", "#ff4b4b"),
                    createSummaryCard("Medium Severity", "0", "#ffd166")
                );
                resultsDashboardView.getChildren().add(summaryLayout);

                VBox healthyBox = new VBox(12);
                healthyBox.setAlignment(Pos.CENTER);
                healthyBox.setPadding(new Insets(40, 20, 40, 20));
                healthyBox.setStyle("-fx-background-color: #111819; -fx-background-radius: 8; -fx-border-color: #1e2526; -fx-border-radius: 8; -fx-border-width: 1;");
                
                Label checkIcon = new Label("✓");
                checkIcon.setStyle("-fx-font-size: 48px; -fx-text-fill: #06d6a0;");
                
                Label healthyTitle = new Label("No Defects Identified");
                healthyTitle.setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: #dde4e5;");
                
                Label healthyText = new Label("The AI analyzer did not detect any structural design defects or bottlenecks in this category based on the provided metrics and signals.");
                healthyText.setWrapText(true);
                healthyText.setTextAlignment(TextAlignment.CENTER);
                healthyText.setStyle("-fx-font-size: 13px; -fx-text-fill: #849494;");
                
                healthyBox.getChildren().addAll(checkIcon, healthyTitle, healthyText);
                resultsDashboardView.getChildren().add(healthyBox);
            }
        } catch (Exception ex) {
