...
import com.example.anuviya.quality.fix.FixSuggestionEngine;
import com.example.anuviya.ui.WarningCard;
...
public class HealthAnalyserView {
    ...
    private final java.util.function.Supplier<com.example.anuviya.context.AnalysisContext> contextSupplier;

    public HealthAnalyserView(Map<String, EntityViewModel> vmMap) {
        this(vmMap, () -> null);
    }

    public HealthAnalyserView(Map<String, EntityViewModel> vmMap, java.util.function.Supplier<com.example.anuviya.context.AnalysisContext> contextSupplier) {
        this.vmMap = vmMap;
    ...
    private Node metricGrid() {
        ...
        grid.add(metricCard("Methods", (int)currentVm.getMembers().stream().filter(m -> !m.name().equals("<init>")).count(), 15), 1, 0);
