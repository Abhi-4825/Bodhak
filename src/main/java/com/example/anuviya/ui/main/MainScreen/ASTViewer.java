    ...
    private static class ChildrenCollector implements IRVisitor {
        ...
        public void visit(ModuleDeclaration node) {
            ...
            if (node.moduleStatements() != null) {
                children.addAll(node.moduleStatements());
            }
        }
        ...
        @Override
        public void visit(MatchStatement node) {
            if (node.subject() != null) children.add(node.subject());
            if (node.cases() != null) {
                node.cases().forEach(c -> {
                    if (c.pattern() != null) children.add(c.pattern());
                    if (c.body() != null) children.add(c.body());
                });
            }
        }

        @Override
        public void visit(ResourceStatement node) {
            if (node.resources() != null) children.addAll(node.resources());
            if (node.body() != null) children.add(node.body());
        }

        @Override
        public void visit(AwaitExpression node) {
            if (node.expression() != null) children.add(node.expression());
        }
        ...
        @Override
        public void visit(ComprehensionExpression node) {
            if (node.element() != null) children.add(node.element());
            if (node.key() != null) children.add(node.key());
            if (node.generators() != null) {
                node.generators().forEach(g -> {
                    if (g.variable() != null) children.add(g.variable());
                    if (g.iterable() != null) children.add(g.iterable());
                    if (g.conditions() != null) children.addAll(g.conditions());
                });
            }
        }

        @Override
        public void visit(CollectionLiteralExpression node) {
            if (node.elements() != null) children.addAll(node.elements());
        }

        @Override
        public void visit(DictionaryExpression node) {
            if (node.entries() != null) {
                node.entries().forEach(entry -> {
                    if (entry.key() != null) children.add(entry.key());
                    if (entry.value() != null) children.add(entry.value());
                });
            }
        }
    }
