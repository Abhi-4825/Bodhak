...
import com.example.bodhak.model.entity.MethodFilter;
...
public class MethodView {
    ...
    private CallableDeclaration previewedMember = null;
        ...
        sourceArea.textProperty().addListener((obs, o, text) -> {
            ...
            for (int i = 0; i < lines; i++) {
                nums.append(i == 0 && previewedMember != null
                        ? previewedMember.sourceRange().startLine()
                        : (previewedMember != null ? previewedMember.sourceRange().startLine() + i : i + 1));
    ...
    private void showSourcePreview(CallableDeclaration m) {
        previewedMember = m;

        boolean isConstructor = m.name().equals("<init>");
        String kindTag = isConstructor ? "CONSTRUCTOR" : "METHOD";
        String returnStr = isConstructor ? "" : (" → " + (m.returnType() != null && m.returnType().qualifiedName() != null && !m.returnType().qualifiedName().toString().isBlank() ? m.returnType().qualifiedName().toString() : "void"));

        previewTitle.setText(m.name() + buildParamStr(m) + returnStr);

        String vis = m.modifiers().stream()
        ...
        int stmtCount = 0;
        if (m.body() instanceof com.example.bodhak.ir.statement.BlockStatement bs) {
            stmtCount = bs.statements().size();
        }
        previewMeta.setText(kindTag + "  ·  " + vis + "  ·  Lines " + m.sourceRange().startLine() + "–" + m.sourceRange().endLine()
    ...
    private String extractSourceBody(CallableDeclaration m) {
        try {
            List<String> allLines = Files.readAllLines(currentVm.getSourceFile().toPath());
            int start = Math.max(0, m.sourceRange().startLine() - 1);
            int end   = Math.min(allLines.size(), m.sourceRange().endLine());
    ...
    private String buildParamStr(CallableDeclaration m) {
        if (m.parameters() == null || m.parameters().isEmpty()) return "()";
        return "(" + m.parameters().stream()
    ...
    private void bind(String entityName) {
        ...
        List<CallableDeclaration> members = currentVm != null ? new ArrayList<>(currentVm.getMembers()) : List.of();
    ...
    private final ListChangeListener<CallableDeclaration> memberListener =
    ...
    private void rebuild() {
        ...
        List<CallableDeclaration> items = applyFilter(activeFilter, members);
    ...
    private Node buildFilterBar(List<CallableDeclaration> members) {
        long totalMethods = members.stream().filter(m -> !m.name().equals("<init>")).count();
        long totalCtors   = members.stream().filter(m -> m.name().equals("<init>")).count();
        ...
        long empty        = members.stream().filter(m -> m.body() == null || !(m.body() instanceof com.example.bodhak.ir.statement.BlockStatement bs) || bs.statements().isEmpty()).count();
    ...
    private Node buildMemberCard(CallableDeclaration m) {
        boolean isConstructor = m.name().equals("<init>");
        ...
        Label kindBadge = new Label(isConstructor ? "CTOR" : "MTH");
        ...
        String returnStr = isConstructor ? "void" : (m.returnType() != null && m.returnType().qualifiedName() != null && !m.returnType().qualifiedName().toString().isBlank() ? m.returnType().qualifiedName().toString() : "void");
        ...
        int linesCount = m.sourceRange().endLine() - m.sourceRange().startLine() + 1;
        metrics.getChildren().addAll(
                miniMetric("STMTS", String.valueOf(stmtCount)),
                miniMetric("LINES", String.valueOf(linesCount)),
                miniMetric("DEPS",  "0")
        );
        ...
        header.setOnMouseClicked(e -> {
            if (e.getTarget() != expandBtn && e.getTarget() != previewBtn) {
                uiFeatures.openAndHighlight(
                        m.name(), m.sourceRange().startLine(), m.sourceRange().startColumn(), currentVm.getSourceFile()
                );
            }
        });
    ...
    private String buildParamStr_signature(CallableDeclaration m) {
        if (m.parameters() == null || m.parameters().isEmpty()) return m.name() + "()";
        return m.name() + "(" + m.parameters().stream()
                .map(p -> (p.type() != null ? p.type().qualifiedName().toString() : "") + " " + p.name())
                .collect(Collectors.joining(", ")) + ")";
    }
    ...
    private VBox buildExpandedDetails(CallableDeclaration m) {
        ...
        if (m.parameters() != null && !m.parameters().isEmpty()) {
            panel.getChildren().add(detailSection("PARAMETERS",
                    m.parameters().stream().map(p -> (p.type() != null ? p.type().qualifiedName().toString() : "") + "  " + p.name()).collect(Collectors.toList()),
                    "param-item"));
        }
        return panel;
    }
    ...
    private List<CallableDeclaration> applyFilter(MethodFilter filter, List<CallableDeclaration> members) {
        return switch (filter) {
            ...
            case EMPTY             -> members.stream().filter(m -> m.body() == null || !(m.body() instanceof com.example.bodhak.ir.statement.BlockStatement bs) || bs.statements().isEmpty()).toList();
