...
public record ProjectClassificationResult(
        ...
        /** All detected technologies with their evidence. */
        List<DetectedTechnology> detectedTechnologies,

        /** The rule that matched the primary project type (used for reasoning). */
        ClassificationRule primaryRule,

        /** Duration of classification inference in milliseconds. */
        double inferenceTimeMs
) {
    public ProjectClassificationResult(
            Map<ProjectType, Double> projectTypes,
            ProjectType primaryType,
            CapabilityProfile capabilityProfile,
            List<FrameworkDetectionResult> detectedFrameworks,
            List<DetectedTechnology> detectedTechnologies) {
        this(projectTypes, primaryType, capabilityProfile, detectedFrameworks, detectedTechnologies, null, 0.0);
    }
    ...
    public ProjectClassificationResult(
            ...
            CapabilityProfile capabilityProfile,
            List<FrameworkDetectionResult> detectedFrameworks) {
        this(projectTypes, primaryType, capabilityProfile, detectedFrameworks, List.of(), null, 0.0);
    }
