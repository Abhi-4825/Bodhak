...
public class RefactoringSuggestionEngine {
    ...
    private List<RefactoringSuggestion> findLongFunctions(AnalysisContext context,Metrics metrics) {
        ...
        return suggestions.stream()
                .sorted((a, b) -> {
                    int bCount = 0;
                    if (b.getMethod().body() instanceof com.example.bodhak.ir.statement.BlockStatement bbs) {
                        bCount = bbs.statements().size();
                    }
                    int aCount = 0;
                    if (a.getMethod().body() instanceof com.example.bodhak.ir.statement.BlockStatement abs) {
