...
public class DependencyGraph {
    ...
    public DependencyGraph(LanguagePluginRegistry registry) {
        this.registry = registry;
    }

    // ── Write-side API ────────────────────────────────────────────────────────

    public void buildInitialGraph(Map<Path, Set<String>> pathNames, Set<String> allKnownNames) {
        fileDependencies.clear();
        for (Map.Entry<Path, Set<String>> entry : pathNames.entrySet()) {
            Path file = entry.getKey();
            LanguagePlugin plugin = registry.forFile(file).orElse(null);
            if (plugin != null) {
                DependencyResolver resolver = plugin.getDependencyResolver();
                Map<String, Set<String>> deps = resolver.resolve(file, allKnownNames);
    ...
    public void updateForFile(Path file, Set<String> knownNames) {
        Path normalized = file.toAbsolutePath().normalize();
        LanguagePlugin plugin = registry.forFile(normalized).orElse(null);
        if (plugin != null) {
            Map<String, Set<String>> deps = plugin.getDependencyResolver().resolve(normalized, knownNames);
            fileDependencies.put(normalized, deps);
            recomputeGlobals();
        }
    }
