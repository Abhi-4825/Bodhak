...
public class BetweennessCentralityEvidenceBuilder {

    public List<BetweennessCentralityEvidence> buildCentralityEvidence(AnalysisContext context) {

        GraphSnapshot snapshot = context.getDependencyGraph().snapshot();

        Map<String, Set<String>> graph = buildDirectedGraph(snapshot);
        ...
        return centrality.entrySet().stream().map(entry -> new BetweennessCentralityEvidence(entry.getKey(), entry.getValue())).sorted(Comparator.comparingDouble(BetweennessCentralityEvidence::score).reversed()).toList();
    }

    private Map<String, Set<String>> buildDirectedGraph(
